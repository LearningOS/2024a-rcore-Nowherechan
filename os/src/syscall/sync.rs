...
pub fn sys_mutex_create(blocking: bool) -> isize {
        ...
        .map(|(id, _)| id)
    {
        ...
        process_inner.used_mutex_list[id] = false;
        id as isize
    } else {
        process_inner.mutex_list.push(mutex);
        process_inner.used_mutex_list.push(false);
...
pub fn sys_mutex_lock(mutex_id: usize) -> isize {
    ...
    let process = current_process();
    let mut process_inner = process.inner_exclusive_access();
    // Lab ch8 -- deadlock detect
    if process_inner.deadlock_detect_enabled && process_inner.used_mutex_list[mutex_id] {
        return -0xdead;
    } else {
        process_inner.used_mutex_list[mutex_id] = true;
    }
...
pub fn sys_mutex_unlock(mutex_id: usize) -> isize {
    ...
    let mut process_inner = process.inner_exclusive_access();
    // Lab ch8 -- deadlock detect
    process_inner.used_mutex_list[mutex_id] = false;
    let mutex = Arc::clone(process_inner.mutex_list[mutex_id].as_ref().unwrap());
    drop(process_inner);
    drop(process);
    mutex.unlock();
...
pub fn sys_semaphore_create(res_count: usize) -> isize {
        ...
        .map(|(id, _)| id)
    {
        ...
        for tid in 0..process_inner.tasks.len() {
            process_inner.semaphore_allocation[tid][id] = 0;
            process_inner.semaphore_need[tid][id] = 0;
        }
        id
    } else {
        process_inner
...
pub fn sys_semaphore_down(sem_id: usize) -> isize {
    ...
    if process_inner.deadlock_detect_enabled {
        ...
        loop {
            ...
            let mut found = false;
            let mut tid_found = 0;
            for tid in 0..task_cnt {
                if finish[tid] {
                    continue;
                }
                let mut this_tid_valid = true;
                for sem_id in 0..sem_cnt {
                    if need[tid][sem_id] > work[sem_id] {
                        this_tid_valid = false;
                    }
                }
                if this_tid_valid {
                    found = true;
            ...
            if found {
                // step 3
                for sem_id in 0..sem_cnt {
                    work[sem_id] += process_inner.semaphore_allocation[tid_found][sem_id];
                }
                finish[tid_found] = true;
            } else {
                break;
            }
        }
